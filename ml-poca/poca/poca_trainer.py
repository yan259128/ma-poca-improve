...
from poca_networks  import SimpleActor, SharedActorCritic
...
class POCATrainer(OnPolicyTrainer):
    ...
    def __init__(
        ...
        """
        负责收集经验和培训POCA模型。
        :param behavior_name: 与培训师配置关联的行为的名称
        :param reward_buff_cap: 要在奖励缓冲区中跟踪的最大奖励历史记录
        :param trainer_settings: 培训师的参数。
        :param training: 培训师是否已做好培训准备。
        :param load: 是否应加载模型。
        :param seed: 将使用种子初始化模型
        :param artifact_path: 在其中存储来自该培训器的工件的目录。
        """
        super().__init__(
            behavior_name,
            reward_buff_cap,
    ...
    def _process_trajectory(self, trajectory: Trajectory) -> None:
        """
        获取轨迹并进行处理，将其放入更新缓冲区。
        处理包括计算模型更新步骤的价值和优势目标。
        :param trajectory: 包含要处理的步骤的“轨迹”元组。
        """
